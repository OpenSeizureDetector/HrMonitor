...
public class HrMonitorService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        String hrmAddrStr = SP.getString("HrmAddr", "10");
        Log.v(TAG, "onStart() - hrmAddrStr = " + hrmAddrStr);


        if (initialize()) {
            Log.v(TAG,"initialize() complete - connecting");
            if (connect(hrmAddrStr)) {
                Log.v(TAG,"connect() successful");
            } else {
    ...
    private void broadcastUpdate(final String action,
        ...
        if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {
            ...
            mHrmDb.insertReading(heartRate,"note");
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                for (BluetoothGattService gattService : gatt.getServices()) {
                    // Loops through available Characteristics and subscribes to heart rate notifications
                    for (BluetoothGattCharacteristic gattCharacteristic : gattService.getCharacteristics()) {
                        if (UUID_HEART_RATE_MEASUREMENT.equals(gattCharacteristic.getUuid())) {
                            Log.v(TAG,"onServicesDiscovered() - found HRM");
